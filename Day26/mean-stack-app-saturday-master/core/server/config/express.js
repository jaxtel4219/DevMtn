...
var express = require('express'),
    ...
    config = require('./config'); //we require this so we can choose what middleware to use (ie: heroku vs development)


module.exports = function () {
 //grab and create exports application

    var app = express();
    // MIDDLEWARE NEEDED BOTH FOR DEV AND PRODUCTION

    // fixes cross-origin issues
    app.use(cors()); //takes care of headers (at the cost of security)
    ...
    if (process.env.NODE_ENV === 'development') {
        app.use(morgan('dev')); //this is a logger for http call feedback
    }
